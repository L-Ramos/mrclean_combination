...
class BrainSegmentationDataset(Dataset):
    ...
    def __init__(self,path_data, idx_samples,data,label_name,transform=None,cols_use=None,only_clin=False,
                 batch_size=8, dim=(256, 256,30),
                 n_channels=1, n_classes=2, shuffle=True,conv_type = '2D'):
        """Initialization
    ...
    def __len__(self):
        ...
        """
        return len(self.idx_samples)
        #return int(np.floor(len(self.idx_samples) / self.batch_size))
    ...
    def __getitem__(self, index):
        ...
        #indexes_samples = self.idx_samples[index * self.batch_size:(index + 1) * self.batch_size]
        idx = self.idx_samples[index]

        # Find list of IDs
        #indexes_samples_temp = [self.indexes_samples[k] for k in indexes_samples]
        #indexes_labels_temp = [self.indexes_labels[k] for k in indexes_labels]

        # Generate data
        
        if self.only_clin:
            z = self._generate_z(idx)
            y = self._generate_y(idx)
            z = torch.from_numpy(z.astype(np.float32))
            y = torch.from_numpy(y.astype(np.float32))
            return z,y

        X = self._generate_X(idx)
        y = self._generate_y(idx)
    ...
    #     """
    #     self.indexes = np.arange(len(self.path_data))
    #     if self.shuffle == True:
    #         np.random.shuffle(self.indexes)

    def _generate_X(self, ID):
        ...
        #X = self._load_image(os.path.join(self.path_data,"s_"+str(ID)+".npy"))  
        #X = np.expand_dims(X, axis=0)
            #print("X=",ID)
            
        if self.conv_type=='3D':            
            X = self._load_image(os.path.join(self.path_data, ID+".npy"))            
            X = np.expand_dims(X, axis=0)
        else:
            X = self._load_image(os.path.join(self.path_data, ID+".npy"))            
    ...
    def _generate_y(self, ID):
        ...
        y = np.array(self.data[self.data['s_id']==ID][self.label_name].iloc[0],dtype='int16')
        y = np.expand_dims(y, axis=-1)
        #return tf.keras.utils.to_categorical(y, self.n_classes)
        return(y)
    
    def _generate_z(self, ID):
        """Generates data containing batch_size clinical data
        :param list_IDs_temp: list of label ids to load
        :return: batch if masks
        """
           
        z = self.data[self.data['s_id']==ID][self.cols_use]
        z = np.expand_dims(z, axis=-1)
        #z = np.array(z)
        return z
    
    def _load_image(self, path_data):
